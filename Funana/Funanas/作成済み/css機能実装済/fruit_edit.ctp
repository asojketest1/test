...
function delFruit(itemid){
    document.getElementById('fruitForm').action = "http://localhost/cake3app/funana/deleteFruit?item="+itemid;
}
$(function(){
    var index = 1;
    $('#btn').click(function(){
        ...
        tr_element.innerHTML =  '<?= $this->Form->hidden("ID",["value"=>$id]) ?>'+
                                ...
                                '<td class="inscontent"><?=$this->Form->text("CONTENT", ['style'=>"width:20em; padding:1em;", 'class'=>'editFTxtbox', ['size'=>100]]) ?></td>';
...
<?=$this->Form->create(null,['url'=>['action'=>'fruit'], 'id'=>'fruitForm']) ?>
    ...
    <table class="tbfruit" id="editFruitTable">
        ...
        <tr>
            <h1>'-'を押した時下の行から削除される</h1>
        <?php foreach($data as $obj): ?>
            <tr>
                <?= $this->Form->hidden("ID",["value"=>$id]) ?>
                <?= $this->Form->hideen("RECORD_ID",["value"=>$record]) ?>
                ...
                <td>
                    <?=$this->Form->button("－",['type'=>'submit', 'div'=>'false', 'class'=>'delBtn', 'onclick'=>"delFruit($obj->ITEM_ID);"])?>
                </td>
            </tr>
        <?php endforeach; ?>
